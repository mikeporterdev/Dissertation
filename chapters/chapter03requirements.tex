...
Both users have different objectives when using the application, and it is important that the requirements of each user are analysed separately to ensure that the application is suitable for both.
...
Different actors will have different objectives for using the application and it is important that the requirements of each actor are analysed to ensure the application is suitable. %you have pretty much exactly repeated yourself here. Seems like here is the more relevent place for that info, so maybe delete the first pragraph in this section and stick in any missing bits here?
In this project there are two key actors, the Child and the Parent. 
The child uses the application to track their tasks, and the parent uses the application to manage both the tasks and the child's progress. %I think they way I worded it in your other chapter might have been clearer. Maybe I didn't change it. But I think you should either exactly match the wording in your Terminology bit, or totally differ it. if that makes sense!
